...
def convert_to_bitarray(list_tuples, window_size, lookahead_size):
  length_window = int(math.log(window_size, 2)) + 1
  length_lookahead = int(math.log(lookahead_size, 2)) + 1
  ...
  for item in list_tuples:
    ...
    mini_array = mini_array + "{0:b}".format(item[1]).zfill(length_lookahead)
...
def lz77_image_compressor(file_name, window_size, lookahead_buffer):
  try:
...
"""""
with open("image_files/dogsmall.jpg", "rb") as imageFile:
  f = imageFile.read()
  b = bytearray(f)
...
lz77_image_compressor("image_files/dogsmall.jpg", 50, 50)
"""